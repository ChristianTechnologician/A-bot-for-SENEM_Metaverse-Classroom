...
public class VoiceProcessor : MonoBehaviour
{
   ...
   void Awake()
    ...
    public void StartRecording(int sampleRate = 16000, int frameSize = 512, bool? autoDetect = null)
    {
        ...
        Debug.Log("VoiceProcessor: Recording started with sample rate " + sampleRate + " and frame size " + frameSize);
  ...
  /*  /// <summary>
    ...
    /// </summary>
    private Queue<short[]> audioQueue = new Queue<short[]>();

    public BufferManager bufferManager; // Referenza a BufferManager

    private string lastProcessedPhrase = "";
    ...
    IEnumerator RecordData()
    {
        ...
        while (IsRecording)
        {
            ...
            else
            {
                ...
                if (maxVolume >= _minimumSpeakingSampleValue)
                {
                    ...
                    Debug.Log("VoiceProcessor: Audio detected with max volume " + maxVolume);
                }
                else
                {
                    _transmit = false;
                    ...
                    if (_audioDetected && Time.time - _timeAtSilenceBegan > _silenceTimer)
                    {
                        ...
                        Debug.Log("VoiceProcessor: Silence detected.");
                    }
                }
            }
            ...
            if (_audioDetected)
            {
                ...
                // Verifica che la frase non sia già stata processata
                string currentPhrase = ConvertBufferToString(pcmBuffer); // Questo metodo deve convertire il buffer in una stringa
                if (currentPhrase != lastProcessedPhrase)
                {
                    lastProcessedPhrase = currentPhrase;
                    if (bufferManager != null)
                    {
                        bufferManager.AddToBuffer(pcmBuffer);
                        Debug.Log("VoiceProcessor: Frame captured and added to BufferManager.");
                    }
                    else
                    {
                        Debug.LogWarning("VoiceProcessor: BufferManager is not assigned.");
                    }

                    if (OnFrameCaptured != null && _transmit)
                    {
                        OnFrameCaptured.Invoke(pcmBuffer);
                        Debug.Log("VoiceProcessor: Frame captured.");
                    }
                }
            }
            else
        ...
        if (RestartRecording != null)
        {
            ...
            Debug.Log("VoiceProcessor: Recording restarted.");
        }
    }

    private string ConvertBufferToString(short[] pcmBuffer)
    {
        // Implementa qui la logica per convertire il buffer PCM in una stringa
        return string.Join(" ", pcmBuffer);
    }*/
